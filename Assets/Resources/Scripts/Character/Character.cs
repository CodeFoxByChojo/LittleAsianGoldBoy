...
{
    /// <summary>
    ///     Character handles all character based stats. It integrates the Mother class with the MotherEvent class and the
    ///     School class.
    /// </summary>
    public class Character : CountableClass
    {
        ...
        private readonly Mother mother = Mother.GetInstance();
        private readonly School school = School.GetInstance();
        ...
        {
            if (school.IsSchoolActive()) knowledge = knowledge + gameManager.GetConfigData().KnowledgePerHour;
        }

        public bool IsCharacterWorking()
        {
            if (mother.GetMotherEvent() == null) return false;
        ...
        {
            if (amount > knowledge) return false;
        ...
        /// <summary>
        ///     Method of Character, which sells all the gold of the player and adds the money to the money of the Character
        ///     obeject.
        /// </summary>
        /// <returns>Returns the Amount of gold sold and the recieved Money as an int[]</returns>
        public long[] SellGold()
        {
            var a = gold * gameManager.GetGameState().GetCurrentGoldPrice();
            a = a * 100;
            var b = (long) a;
            b = b / 10000;
            long[] result = {gold, b};
        ...
        public bool UpgradeClick()
        {
            if (gameManager.GetCharacter()
                .TakeMoney(gameManager.GetUpgradeCost(clickLevel, gameManager.GetConfigData().ClickBaseCost)))
            {
                clickLevel++;
                return true;
            }
